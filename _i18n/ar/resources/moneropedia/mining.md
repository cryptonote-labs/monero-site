...
Mining is the distributed process of confirming transactions on the public ledger of all transactions, aka @blockchain.  Masari Supreme nodes use the blockchain to distinguish legitimate transactions from attempts to re-spend coins that have already been spent elsewhere.

Masari Supreme is powered strictly by Proof of Work. It employs a mining algorithm that has the potential to be efficiently tasked to billions of existing devices (any modern x86 CPU and many GPUs). Masari Supreme uses the CryptoNight Proof of Work (PoW) algorithm, which is designed for use in ordinary CPUs and GPUs.
